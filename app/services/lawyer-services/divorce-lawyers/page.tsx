...
export const metadata = {
  title: 'Divorce Lawyers - Legal Door',
  description: 'Expert divorce and maintenance legal services.',
};

const services = [
  { title: 'Mutual Divorce', description: 'Mutual divorce consultation and filing.' },
  { title: 'Contested Divorce', description: 'Contested divorce representation.' },
  { title: 'Alimony & Maintenance', description: 'Alimony and maintenance matters.' },
];

export default function DivorceLawyersPage() {
  return (
    <main className="min-h-screen">
      <Navbar />
      <div className="container mx-auto py-24 px-4 pt-32">
        <div className="max-w-4xl mx-auto text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-6"><span className="text-primary">Divorce</span> Lawyers</h1>
          <p className="text-lg text-slate-600 leading-relaxed font-medium">
            Expert divorce legal services including mutual and contested divorce.
          </p>
        </div>
          ...
          {services.map((service) => (
            <div key={service.title} className="bg-white border border-slate-100 p-8 rounded-[40px] hover:shadow-2xl transition-all duration-500 flex flex-col group">
              <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center mb-8 group-hover:bg-primary/10 transition-colors">
                <Heart className="w-7 h-7 text-slate-900 group-hover:text-primary" />
              </div>
