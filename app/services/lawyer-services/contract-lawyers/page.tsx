...
export const metadata = {
  title: 'Contract Lawyers - Legal Door',
  description: 'Expert contract drafting and review services.',
};

const services = [
  { title: 'Contract Drafting', description: 'Professional drafting of legal contracts.' },
  { title: 'Contract Review', description: 'Thorough review of existing contracts.' },
  { title: 'Contract Dispute', description: 'Resolution of contract disputes.' },
];

export default function ContractLawyersPage() {
  return (
    <main className="min-h-screen">
      <Navbar />
      <div className="container mx-auto py-24 px-4 pt-32">
        <div className="max-w-4xl mx-auto text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-6">Contract <span className="text-primary">Lawyers</span></h1>
          <p className="text-lg text-slate-600 leading-relaxed font-medium">
            Expert contract drafting, review and dispute resolution services.
          </p>
        </div>
          ...
          {services.map((service) => (
            <div key={service.title} className="bg-white border border-slate-100 p-8 rounded-[40px] hover:shadow-2xl transition-all duration-500 flex flex-col group">
              <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center mb-8 group-hover:bg-primary/10 transition-colors">
                <Briefcase className="w-7 h-7 text-slate-900 group-hover:text-primary" />
              </div>
