...
export const metadata = {
  title: 'Criminal Lawyers - Legal Door',
  description: 'Expert criminal defence and bail services.',
};

const services = [
  { title: 'Criminal Defense', description: 'Expert defence in criminal cases.' },
  { title: 'Bail Application', description: 'Bail application and hearing services.' },
  { title: 'FIR Quashing', description: 'Legal remedies for FIR quashing.' },
];

export default function CriminalLawyersPage() {
  return (
    <main className="min-h-screen">
      <Navbar />
      <div className="container mx-auto py-24 px-4 pt-32">
        <div className="max-w-4xl mx-auto text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-6">Criminal <span className="text-primary">Lawyers</span></h1>
          <p className="text-lg text-slate-600 leading-relaxed font-medium">
            Expert criminal defence and bail services.
          </p>
        </div>
          ...
          {services.map((service) => (
            <div key={service.title} className="bg-white border border-slate-100 p-8 rounded-[40px] hover:shadow-2xl transition-all duration-500 flex flex-col group">
              <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center mb-8 group-hover:bg-primary/10 transition-colors">
                <Gavel className="w-7 h-7 text-slate-900 group-hover:text-primary" />
              </div>
